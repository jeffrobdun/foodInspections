...
if(isset($_POST['letter'])){
	...
	if($_POST['letter'] == "0-9"){
		$selectResturantByLeter = 'SELECT * FROM establishmentHistory WHERE name REGEXP \'^[0-9]\';';
	}else{
		$selectResturantByLeter = 'SELECT * FROM establishmentHistory WHERE name LIKE \'' . $db->real_escape_string($letter) .'%\';';
	}
	...
	$selectResturant = 'SELECT * FROM establishmentHistory'; //'WHERE name LIKE \'%' . $db->real_escape_string($resturantName) .'%\' AND address LIKE \'%' . $db->real_escape_string($address) . '%\' AND inspectionDate LIKE \'%' . $db->real_escape_string($inspectionDate) . '%\' AND city LIKE \'%' . $db->real_escape_string($city) . '%\'';
